...
const StyledTab = styled((props) => <Tab disableRipple {...props} />)(({ theme, colorOnHover }) => ({
...
//This function is responsible for handling changes in the tab selection.
  ...
  const handleTermChange = (event) => {
    ...
    allCourseDetails.forEach(singleCourse => {
        ...
        {
          var entity = {'name': singleCourse['name'], 'id':singleCourse['id'], 'role':singleCourse['role'] }
  ...
  return (
      <ThemeProvider theme={mdTheme}>
        <Box sx={{ display: 'flex'}}>
          <CssBaseline />
          <AppBar style={{ background: "#4A2E83" }} position="absolute" open={open}>
            <Toolbar
              sx={{
             ...
             {show && <IconButton
              ...
              <div>
                <FormControl fullWidth={true} sx={{ m: 1, minWidth: 200, color: 'white'}}>
                  <InputLabel sx={{ backgroundColor:'#4A2E83', color: 'white'}} id="demo-simple-select-label"> Courses </InputLabel>
                  ...
                  >
                     {favoriteCourses.map(course => (
                      <MenuItem
                        ...
                        value={course}
                      >
                        <FavoriteIcon style={{ color:'#4A2E83'}}/>&ensp;{course.name}
                      </MenuItem>
                    ))}                
                    <Divider />
                    {coursesInTerm.map(courseEntity => (
                      <MenuItem
                        key = {courseEntity.name}
                        value={courseEntity}>
                       {courseEntity.name}
                      </MenuItem>
                    ))} 
              ...
              <div>
                <FormControl fullWidth={true} sx={{ m: 1, minWidth: 200, color: 'white'}}>
                  <InputLabel sx={{ backgroundColor:'#4A2E83', color: 'white'}} id="demo-simple-select-label"> Terms </InputLabel>
                  <Select
                    labelId="demo-simple-select-label"
                    id="demo-simple-select-filled"
           ...
           <LoginButton isLoggedIn={isLoggedIn} setIsLoggedIn={setIsLoggedIn} setShow={setShow}
              onLogout={() => { 
          ...
          <Drawer variant={drawer} open={open}>
            <Toolbar
              sx={{
                ...
                alignItems: 'center',             
                px: [1],
                backgroundColor: selectedTab === 0 ? 'sidebarColor1' : 'sidebarColor2', 
              }}
            >
           <Tabs
  value={selectedTab}
...
<StyledTab
    ...
    sx={{
      backgroundColor: selectedTab === 0 ? 'rgba(76, 175, 80, 0.2)' : 'rgba(76, 175, 80, 0.2)', // 'transparent'Set the background color for the first tab
  ...
  <StyledTab
    ...
    sx={{
      backgroundColor: selectedTab === 1 ? 'rgba(74,46,131, 0.2)' : 'rgba(74,46,131, 0.2)', // Set the background color for the second tab
  ...
  <Divider />
           ...
           <List
           sx={{
            backgroundColor: selectedTab === 0 ? `rgba(76, 175, 80, 0.2)` : 'rgba(74,46,131, 0.2)', // Set the background color for the first tab
            color: selectedTab === 0 ? 'inherit' : 'inherit', // Set the text color for the first tab
          }} 
           >
  {isBatch ? MainListItems(courseId, courseRole, name ) : SecondaryListItems(courseId, courseRole, name )}
</List>
          ...
          <Container maxWidth="lg" sx={{ mt: 4, mb: 4}}>            
              ...
              <Welcome />  //when you logout it will go back to the Welcome page         
               }
