...
swagger_blue_print = get_swaggerui_blueprint(
    ...
    swagger_api_url,
    config={
        'app_name': " Teaching Tools - Software Engineering Studio"
    }
)
...
def save_access_token_info(access_token_response):
    ...
    expiry_time = datetime.datetime.now(pytz.utc) + datetime.timedelta(seconds=access_token_response["expires_in"])
...
def createGoogleFolder():
    ...
    try:
        return {"Folder": status, "Response": ("Folder '" + status["name"] + "' created")}
    except:
        return {"Response": status}
...
def downloadCourseSyllabus():
    ...
    if downloadToLocal == True:
        status = "Uploaded Syllabus file to Google Drive and saved to your Local machine."
    else:
...
def exportQTIQuiz():
    ...
    response = request.get_json(force=True)
    courseId = response.get('courseId')
    courseOption = response.get('courseOption')  # Assuming this is used in the logic
    quizType = response.get('quizType')  # Assuming this is used in the logic

    canvas = get_canvas_instance()
    if canvas is None:
        return jsonify({"error": "Not authenticated"}), 401

    result = canvas.exportQuizzesToQTI(courseId, courseOption, quizType)
    if result == 'success':
        # Assuming the result includes a path or URL to the exported QTI file
        return jsonify(message="QTI Quizzes exported successfully", status_code=200)
    else:
        return jsonify(message="Failed to export QTI Quizzes", status_code=500)
